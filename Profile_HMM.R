...
# Getting genes that are differentially expressed between upper and lower over the whole time series
design(dds) = ~ tooth
dds <- DESeq(dds)
...
# Getting genes that are differentially expressed between upper and lower at each time points (taking consecutive timepoints as replicates)
...
# Merging the lists of DE genes, either over the whole time serie, or per time point
...
# We are working on the differential between upper and lower tooth, per timepoint. 
...
dotchart(as.matrix(shiftsNormal_Tot),col=rainbow(4))
dev.off()
